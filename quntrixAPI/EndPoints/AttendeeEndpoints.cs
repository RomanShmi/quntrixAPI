...
namespace quntrixAPI.EndPoints
{
       ...
       public static class AttendeeEndpoints
    {
        ...
        {
            routes.MapGet("/api/Attendee/{username}", async (string username, AppDbContext db) =>
            {
            ...
            routes.MapPost("/api/Attendee/", async (ConferenceDTO.Attendee input, AppDbContext db) =>
            {
            ...
            routes.MapPost("/api/Attendee/{username}/Session/{sessionId}",
                async (string username, int sessionId, AppDbContext db) =>
                {
                    var attendee = await db.Attendees.Include(a => a.SessionsAttendees)
                                    .ThenInclude(sa => sa.Session)
                                    .SingleOrDefaultAsync(a => a.UserName == username);
